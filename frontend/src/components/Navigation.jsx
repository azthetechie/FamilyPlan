...
export default function Navigation({ user }) {
  ...
  const onActivityPage = location.pathname === '/activity';

  return (
    <header className="sticky top-0 z-40 backdrop-blur-xl bg-[#FDFBF7]/80 border-b-2 border-gray-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between">
        <Link to="/dashboard" className="flex items-center gap-3" data-testid="nav-brand">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <Link
            ...
            to={onActivityPage ? '/dashboard' : '/activity'}
            className="neo-btn bg-white text-gray-900 px-3 py-2 inline-flex items-center gap-2 text-sm"
            title={onActivityPage ? 'Back to dashboard' : 'View activity log'}
          >
            <Activity size={16} strokeWidth={2.5} />
            <span className="hidden sm:inline">{onActivityPage ? 'Dashboard' : 'Activity'}</span>
          </Link>
